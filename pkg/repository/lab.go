...
func (r *Lab) GetAll(p PaginationInput, filter *Lab) ([]Lab, int64, error) {
	...
	dbOp := DB.Scopes(Paginate(&p)).Select("*, count(*) OVER() AS count").Where(filter).Preload("Order").Preload("Order.Payments").Preload("LabType").Preload("RightEyeImages").Preload("LeftEyeImages").Preload("RightEyeSketches").Preload("LeftEyeSketches").Preload("Images").Preload("Documents").Order("id ASC").Find(&result)
...
func (r *Lab) DeleteFile(association string, LabID int, fileID int) error {
	return DB.Model(&Lab{ID: LabID}).Association(association).Delete(&File{ID: fileID})
}

// ClearAssociation ...
func (r *Lab) ClearAssociation(association string, labID int) error {
	return DB.Model(&Lab{ID: labID}).Association(association).Clear()
}
...
func (r *Lab) Delete(ID int) error {
	return DB.Transaction(func(tx *gorm.DB) error {
		...
		var labsCount int64
		if err := tx.Model(&r).Where("lab_order_id = ?", r.LabOrderID).Count(&labsCount).Error; err != nil {
			return err
		}

		if labsCount == 0 {
			if err := tx.Where("id = ?", r.LabOrderID).Delete(&LabOrder{}).Error; err != nil {
				return err
			}
		}
