...
func (r *SurgicalProcedure) GetByPatientChart(ID int) error {
	return DB.Where("patient_chart_id = ?", ID).Preload("SurgicalProcedureType").Preload("PreanestheticDocuments").Take(&r).Error
}

// GetAll ...
func (r *SurgicalProcedure) GetAll(p PaginationInput, filter *SurgicalProcedure) ([]SurgicalProcedure, int64, error) {
	var result []SurgicalProcedure

	dbOp := DB.Scopes(Paginate(&p)).Select("*, count(*) OVER() AS count").Where(filter).Preload("SurgicalProcedureType").Preload("PreanestheticDocuments").Order("id ASC").Find(&result)
...
func (r *SurgicalProcedure) DeleteFile(association string, surgicalProcedureID int, fileID int) error {
	return DB.Model(&SurgicalProcedure{ID: surgicalProcedureID}).Association(association).Delete(&File{ID: fileID})
}
...
func (r *SurgicalProcedure) GetByPatient(p PaginationInput, patientID int) ([]SurgicalProcedure, int64, error) {
	...
	dbOp := DB.Scopes(Paginate(&p)).Joins("INNER JOIN orders ON orders.id = surgical_procedures.order_id").Where("orders.patient_id = ?", patientID).Preload("Order").Preload("Order.Payments").Preload("Order.User").Preload("SurgicalProcedureType").Order("surgical_procedures.id ASC").Find(&result)
...
func (r *SurgicalProcedure) Delete(ID int) error {
	return DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&r).Where("surgical_order_id = ?", r.SurgicalOrderID).Count(&surgicalCount).Error; err != nil {
			return err
		}

		if surgicalCount == 0 {
			if err := tx.Where("id = ?", r.SurgicalOrderID).Delete(&SurgicalOrder{}).Error; err != nil {
				return err
			}
		}
